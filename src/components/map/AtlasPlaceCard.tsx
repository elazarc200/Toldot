...
export function AtlasPlaceCard({place,time,onClose,onSage,fullEntry=false}:{place:AtlasPlace;time:TimeFilter;onClose?:()=>void;onSage?:(id:string)=>void;fullEntry?:boolean}){
 ...
 <h3>חכמים שפעלו במקום</h3>{content.activities.length?content.activities.map(r=>{const sage=sages.find(s=>s.id===r.personId)!;const rel='relationshipType' in r?String((r as {relationshipType?:string}).relationshipType||''):'';return <details className="atlas-activity" key={r.id}><summary><strong>{sage.name}</strong><span>{r.activity}</span></summary><p>{r.note}</p><p className="atlas-muted">{importanceLabels[r.importance as keyof typeof importanceLabels]} · {r.importanceNote}</p>{rel?<p className="atlas-muted">סוג קשר: {relationshipTypeLabels[rel]||rel}</p>:null}<p className="atlas-muted">{r.timeNote}</p><AtlasSources ids={r.sourceIds}/><div className="atlas-inline-links">{onSage?<button onClick={()=>onSage(sage.id)}>התמקדות בחכם במפה</button>:<Link href={`/map?person=${sage.id}`}>התמקדות במפה</Link>}<Link href={`/knowledge?person=${sage.id}`}>כרטיס החכם בעץ ↗</Link></div></details>;}):<p className="atlas-muted">לא תועדו חכמים במקום במסגרת הזמן שנבחרה.</p>}
